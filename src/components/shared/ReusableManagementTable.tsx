...
import { ArrowDown, ArrowUp, ArrowUpDown, Edit, Eye, Loader2, MoreHorizontal, Trash, X } from "lucide-react";
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
function ReusableManagementTable<T>({
    data = [],
    columns = [],
    onView,
    onEdit,
    ...
    return (
        <div className="rounded-lg border relative overflow-x-auto">
            {isRefreshing && (
                            ...
                            <TableHead key={colIndex} className={column.className}>
                                {column.sortKey ? (
                                    <span
                                        ...
                                        className="flex items-center p-2 hover:text-foreground transition-colors font-medium cursor-pointer select-none whitespace-nowrap"
                                    >
                                        {column.header}
...
export default ReusableManagementTable;